	...
	testCases := []struct {
	...
	}{
		{
			...
			sqlQueries: []model.SqlQuery{
				{
					Sql: "SELECT m.metric_name, m.table_schema, array_agg(s.id)\n\t" +
			...
			sqlQueries: []model.SqlQuery{
				{
					...
					Results: model.RowResults{{`foo`, `prom_data`, []int64{1}}},
					Err:     error(nil),
				},
				{
			...
			sqlQueries: []model.SqlQuery{
				{
					...
					Results: model.RowResults{{"foo", "prom_data", []int64{1}}, {"bar", "prom_data", []int64{1}}},
					Err:     error(nil),
				},
				{
					Sql:     "SELECT table_schema, table_name, series_table FROM _prom_catalog.get_metric_table_name_if_exists($1, $2)",
			...
			sqlQueries: []model.SqlQuery{
				{
					Sql: "SELECT m.metric_name, m.table_schema, array_agg(s.id)\n\t" +
						...
						"ORDER BY m.metric_name",
					Args:    []interface{}{"foo", "bar"},
					Results: model.RowResults{{"metric", "prom_data", []int64{1, 99, 98}}},
					Err:     error(nil),
				},
				{
					Sql:     "SELECT table_schema, table_name, series_table FROM _prom_catalog.get_metric_table_name_if_exists($1, $2)",
			...
			sqlQueries: []model.SqlQuery{
				{
					...
					Results: model.RowResults{{"metric", "prom_data", []int64{1, 4, 5}}},
					Err:     error(nil),
				},
				{
					Sql:     "SELECT table_schema, table_name, series_table FROM _prom_catalog.get_metric_table_name_if_exists($1, $2)",
			...
			sqlQueries: []model.SqlQuery{
				{
					...
					Results: model.RowResults{{"metric", "prom_data", []int64{1, 2}}},
					Err:     error(nil),
				},
				{
					Sql:     "SELECT table_schema, table_name, series_table FROM _prom_catalog.get_metric_table_name_if_exists($1, $2)",
	...
	for _, c := range testCases {
		t.Run(c.name, func(t *testing.T) {
			...
			mockMetrics := &model.MockMetricCache{
				MetricCache: make(map[string]model.MockCacheEntry),
			}
			err := mockMetrics.Set("prom_data", "metric_1", "prom_data", "metricTableName_1", "metric_1")
			if err != nil {
