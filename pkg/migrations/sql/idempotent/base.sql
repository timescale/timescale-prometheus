...
AS $func$
   SELECT id, table_name::name, table_schema::name, series_table::name, is_view
   FROM SCHEMA_CATALOG.metric m
...
BEGIN
    ...
    $query$
        SELECT array_agg(potential_series.series_id)
				...
				LEFT JOIN LATERAL(
				  ...
				  WHERE data_exists.series_id = potential_series.series_id AND time >= %3$L
    ...
    $query$, r.table_schema, r.table_name, check_time)
...
SET search_path = pg_temp;
--redundant given schema settings but extra caution for security definers
REVOKE ALL ON FUNCTION SCHEMA_CATALOG.get_confirmed_unused_series(TEXT, BIGINT[], TIMESTAMPTZ) FROM PUBLIC;
...
BEGIN
    ...
    IF schema_name = 'SCHEMA_DATA' THEN
        RAISE EXCEPTION 'cannot register metric view in data schema';
    END IF; 
